-- Incluye el idioma del negocio ('es' | 'en') en el payload del push para que
-- el service worker pueda localizar los textos de acción (Ver/Cerrar) y los
-- fallbacks. Basado en la última versión de la función (deep link a la reserva).
-- Requiere: add_business_language.sql (columna businesses.language).
...
BEGIN
  ...
  -- Idioma del negocio (default 'es' si la columna/fila no está disponible)
  BEGIN
    SELECT COALESCE(b.language, 'es') INTO v_language
    FROM public.businesses b
    WHERE b.id = NEW.business_id;
  EXCEPTION WHEN undefined_column THEN
    v_language := 'es';
  END;
  IF v_language IS NULL OR v_language NOT IN ('es', 'en') THEN
    v_language := 'es';
  END IF;
...
COMMENT ON FUNCTION public.send_push_on_notification() IS
  'Sends a Web Push notification via the send-push-notification Edge Function whenever a new notification is inserted. Deep-links to the specific booking when metadata contains booking_id. Includes the business language (lang) for localized rendering. Reads secrets from Supabase Vault. Uses pg_net for async HTTP calls.';
